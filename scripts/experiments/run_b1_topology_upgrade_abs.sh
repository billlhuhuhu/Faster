...
DATASET="${B1_TOPOLOGY_DATASET:-flickr}"
BACKBONE="${B1_TOPOLOGY_BACKBONE:-nfnet}"
TEXT_ENCODER="${B1_TOPOLOGY_TEXT_ENCODER:-bert}"
VARIANT="${B1_TOPOLOGY_VARIANT:-b1_topology_upgrade}"
SEEDS_STR="${B1_TOPOLOGY_SEEDS:-0}"
read -r -a SEEDS <<< "${SEEDS_STR}"
BUDGETS_STR="${B1_TOPOLOGY_BUDGETS:-100 200 500}"
read -r -a BUDGETS <<< "${BUDGETS_STR}"

# Use dedicated roots so the new topology logic can be validated independently.
CROSS_OUTPUT_ROOT="${B1_TOPOLOGY_CROSS_OUTPUT_ROOT:-artifacts/cross_modal_topology_b1_topology_upgrade}"
SELECTION_OUTPUT_ROOT="${B1_TOPOLOGY_SELECTION_OUTPUT_ROOT:-artifacts/subset_selection_b1_topology_upgrade_abs}"
TRAIN_OUTPUT_ROOT="${B1_TOPOLOGY_TRAIN_OUTPUT_ROOT:-artifacts/subset_train_b1_topology_upgrade_abs}"
REPORT_NAME="${B1_TOPOLOGY_REPORT_NAME:-b1_topology_upgrade_abs}"

CORRECTION_MODE="${B1_TOPOLOGY_CORRECTION_MODE:-bidirectional}"
FUSION_MODE="${B1_TOPOLOGY_FUSION_MODE:-confidence_aware}"
CORRECTION_FUSION_MODE="${B1_TOPOLOGY_CORRECTION_FUSION_MODE:-thresholded_autonomy}"
ENABLE_DIRECTIONAL_CORRECTION_GATE="${B1_TOPOLOGY_ENABLE_DIRECTIONAL_CORRECTION_GATE:-1}"
CORRECTION_GATE_TAU_HIGH="${B1_TOPOLOGY_GATE_TAU_HIGH:-0.6}"
CORRECTION_GATE_TAU_LOW="${B1_TOPOLOGY_GATE_TAU_LOW:-0.3}"
CORRECTION_GATE_TAU_GAP="${B1_TOPOLOGY_GATE_TAU_GAP:-0.15}"

EMBEDDING_TYPE="${B1_TOPOLOGY_EMBEDDING_TYPE:-diffusion}"
DIFFUSION_DIM="${B1_TOPOLOGY_DIFFUSION_DIM:-32}"
DIFFUSION_TIME="${B1_TOPOLOGY_DIFFUSION_TIME:-1.0}"
DIFFUSION_EIG_SOLVER="${B1_TOPOLOGY_DIFFUSION_EIG_SOLVER:-auto}"

ENABLE_LOCAL_NODE_CONFIDENCE="${B1_TOPOLOGY_ENABLE_LOCAL_NODE_CONFIDENCE:-1}"
LOCAL_NODE_CONFIDENCE_MODE="${B1_TOPOLOGY_LOCAL_CONF_MODE:-multi_view}"
LOCAL_CONF_WEIGHT_ENTROPY="${B1_TOPOLOGY_LOCAL_CONF_WEIGHT_ENTROPY:-1.0}"
LOCAL_CONF_WEIGHT_AGREEMENT="${B1_TOPOLOGY_LOCAL_CONF_WEIGHT_AGREEMENT:-1.0}"
LOCAL_CONF_WEIGHT_DIFFUSION="${B1_TOPOLOGY_LOCAL_CONF_WEIGHT_DIFFUSION:-1.0}"
LOCAL_CONF_AGREEMENT_TOPK="${B1_TOPOLOGY_LOCAL_CONF_AGREEMENT_TOPK:-15}"
LOCAL_CONF_AGREEMENT_TYPE="${B1_TOPOLOGY_LOCAL_CONF_AGREEMENT_TYPE:-jaccard}"
LOCAL_CONF_DIFFUSION_HOPS="${B1_TOPOLOGY_LOCAL_CONF_DIFFUSION_HOPS:-2}"
LOCAL_CONF_DIFFUSION_TYPE="${B1_TOPOLOGY_LOCAL_CONF_DIFFUSION_TYPE:-p_vs_p2_cosine}"
LOCAL_CONF_TAU_L="${B1_TOPOLOGY_LOCAL_CONF_TAU_L:-0.25}"
LOCAL_CONF_KAPPA_MIN="${B1_TOPOLOGY_LOCAL_CONF_KAPPA_MIN:-0.05}"
LOCAL_CONF_EPS="${B1_TOPOLOGY_LOCAL_CONF_EPS:-1e-8}"

PROXY_LOSS_TYPE="${B1_TOPOLOGY_PROXY_LOSS_TYPE:-diffusion_ms_swd}"
PROXY_INIT_METHOD="${B1_TOPOLOGY_PROXY_INIT_METHOD:-kmeans}"
PROXY_LR="${B1_TOPOLOGY_PROXY_LR:-0.05}"
PROXY_REG_WEIGHT="${B1_TOPOLOGY_PROXY_REG_WEIGHT:-0.01}"
ENABLE_WAVELET_MULTISCALE="${B1_TOPOLOGY_ENABLE_WAVELET_MULTISCALE:-1}"
USE_DPP="${B1_TOPOLOGY_USE_DPP:-1}"
WAVELET_SCALES="${B1_TOPOLOGY_WAVELET_SCALES:-1,2,4}"
WAVELET_DISTANCE_TYPE="${B1_TOPOLOGY_WAVELET_DISTANCE_TYPE:-swd}"
WAVELET_SCHEDULE="${B1_TOPOLOGY_WAVELET_SCHEDULE:-coarse_to_fine}"
WAVELET_LOSS_WEIGHT="${B1_TOPOLOGY_WAVELET_LOSS_WEIGHT:-0.1}"
SWD_NUM_PROJECTIONS="${B1_TOPOLOGY_NUM_PROJECTIONS:-64}"
SWD_P="${B1_TOPOLOGY_SWD_P:-2.0}"
SWD_USE_FIXED_PROJECTIONS="${B1_TOPOLOGY_SWD_USE_FIXED_PROJECTIONS:-0}"
WAVELET_SWD_NUM_PROJECTIONS="${B1_TOPOLOGY_WAVELET_NUM_PROJECTIONS:-64}"
WAVELET_SWD_P="${B1_TOPOLOGY_WAVELET_P:-2.0}"

ENABLE_LSRC="${B1_TOPOLOGY_ENABLE_LSRC:-1}"
LSRC_K="${B1_TOPOLOGY_LSRC_K:-32}"
LSRC_TAU_R="${B1_TOPOLOGY_LSRC_TAU_R:-1.0}"
LSRC_TAU_C="${B1_TOPOLOGY_LSRC_TAU_C:-1.0}"
LSRC_ETA="${B1_TOPOLOGY_LSRC_ETA:-0.5}"
LSRC_BETA="${B1_TOPOLOGY_LSRC_BETA:-0.5}"
LSRC_BATCH_SIZE="${B1_TOPOLOGY_LSRC_BATCH_SIZE:-4096}"
LSRC_COVERAGE_MODE="${B1_TOPOLOGY_LSRC_COVERAGE_MODE:-mean}"
LSRC_REL_LOSS_MODE="${B1_TOPOLOGY_LSRC_REL_LOSS_MODE:-weight_mean}"
LSRC_USE_GLOBAL_CONFIDENCE="${B1_TOPOLOGY_LSRC_USE_GLOBAL_CONFIDENCE:-0}"

LAMBDA_DIFF="${B1_TOPOLOGY_LAMBDA_DIFF:-1.0}"
LAMBDA_MS="${B1_TOPOLOGY_LAMBDA_MS:-0.0}"
LAMBDA_LSRC="${B1_TOPOLOGY_LAMBDA_LSRC:-0.1}"
LSRC_MU="${B1_TOPOLOGY_LSRC_MU:-0.5}"
LAMBDA_REG="${B1_TOPOLOGY_LAMBDA_REG:-1.0}"
REG_ALPHA_DIV="${B1_TOPOLOGY_REG_ALPHA_DIV:-1.0}"
REG_BETA_TOPO="${B1_TOPOLOGY_REG_BETA_TOPO:-1.0}"
REG_GAMMA_INIT="${B1_TOPOLOGY_REG_GAMMA_INIT:-1.0}"

MATCHING_COST_MODE="${B1_TOPOLOGY_MATCHING_COST_MODE:-candidate_topk}"
COST_ALPHA_DIFF="${B1_TOPOLOGY_COST_ALPHA_DIFF:-1.0}"
COST_BETA_WAVELET="${B1_TOPOLOGY_COST_BETA_WAVELET:-0.25}"
COST_GAMMA_TOPO="${B1_TOPOLOGY_COST_GAMMA_TOPO:-0.1}"
COST_ETA_LSRC="${B1_TOPOLOGY_COST_ETA_LSRC:-0.1}"
...
run_precompute_if_needed() {
  ...
  if [[ "${ENABLE_DIRECTIONAL_CORRECTION_GATE}" == "1" ]]; then
    cross_extra_args+=(--enable_directional_correction_gate)
  else
    cross_extra_args+=(--disable_directional_correction_gate)
  fi
  ...
  python "${PROJECT_ROOT}/run_cross_modal_topology.py" \
    ...
    --tau_l "${LOCAL_CONF_TAU_L}" \
    --kappa_min "${LOCAL_CONF_KAPPA_MIN}" \
...
stage_log "B1 topology-upgrade absolute-budget sweep start: dataset=${DATASET} budgets=${BUDGETS[*]} seeds=${SEEDS[*]} variant=${VARIANT}"
...
stage_log "B1 topology-upgrade absolute-budget sweep completed. Logs saved to ${RUN_LOG_DIR}"
