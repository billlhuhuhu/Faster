...
DATASET="${IMAGE_FEATURE_COMPARE_DATASET:-flickr}"
BACKBONE="${IMAGE_FEATURE_COMPARE_BACKBONE:-nfnet}"
TEXT_ENCODER="${IMAGE_FEATURE_COMPARE_TEXT_ENCODER:-bert}"
SEEDS_STR="${IMAGE_FEATURE_COMPARE_SEEDS:-0}"
read -r -a SEEDS <<< "${SEEDS_STR}"
BUDGETS_STR="${IMAGE_FEATURE_COMPARE_BUDGETS:-100 200 500}"
read -r -a BUDGETS <<< "${BUDGETS_STR}"
RATIOS_STR="${IMAGE_FEATURE_COMPARE_RATIOS-0.01 0.02 0.03}"
read -r -a RATIOS <<< "${RATIOS_STR}"

RUN_TIMESTAMP="$(date '+%Y%m%d_%H%M%S')"
REPORT_NAME="${IMAGE_FEATURE_COMPARE_REPORT_NAME:-image_feature_replacement_compare}"
...
run_one_variant() {
  ...
  stage_log "Run image-feature experiment: mode=${image_mode} variant=${variant_name}"
  env \
...
run_one_variant \
  "dense_sift_bovw" \
  "wavelet_main_dense_sift_bovw" \
  "artifacts/feature_cache_dense_sift_bovw" \
  "artifacts/topology_graph_dense_sift_bovw" \
  "artifacts/cross_modal_topology_dense_sift_bovw" \
  "artifacts/subset_selection_dense_sift_bovw" \
  "artifacts/subset_train_dense_sift_bovw" \
  "dense_sift_bovw_combo"

run_one_variant \
  "raw_pixels_pca" \
  "wavelet_main_raw_pixels_pca" \
  "artifacts/feature_cache_raw_pixels_pca" \
  "artifacts/topology_graph_raw_pixels_pca" \
  "artifacts/cross_modal_topology_raw_pixels_pca" \
  "artifacts/subset_selection_raw_pixels_pca" \
  "artifacts/subset_train_raw_pixels_pca" \
  "raw_pixels_pca_combo"

RAW_CSV_PATH="${REPORT_DIR}/image_feature_compare_raw.csv"
SUMMARY_CSV_PATH="${REPORT_DIR}/image_feature_compare_summary.csv"
MISSING_TXT_PATH="${REPORT_DIR}/missing_metrics.txt"

python - "${RAW_CSV_PATH}" "${SUMMARY_CSV_PATH}" "${MISSING_TXT_PATH}" "${DATASET}" "${BACKBONE}" "${TEXT_ENCODER}" "${BUDGETS_STR}" "${RATIOS_STR}" "${SEEDS_STR}" <<'PY'
import csv
import json
import statistics
import sys
...
variants = [
    {
        ...
        "variant": "wavelet_main_dense_sift_bovw",
        "train_root": Path("artifacts/subset_train_dense_sift_bovw"),
    },
    {
        "image_feature_mode": "raw_pixels_pca",
        "variant": "wavelet_main_raw_pixels_pca",
        "train_root": Path("artifacts/subset_train_raw_pixels_pca"),
    },
]
...
stage_log "Image feature replacement compare completed."
