...
DATASET="${LOCALCONF_DATASET:-flickr}"
SEED="${LOCALCONF_SEED:-0}"
BACKBONE="${LOCALCONF_BACKBONE:-nfnet}"
TEXT_ENCODER="${LOCALCONF_TEXT_ENCODER:-bert}"
BUDGETS_STR="${LOCALCONF_BUDGETS:-100 200 500}"
...
LOCALCONF_CROSS_ROOT="${LOCALCONF_CROSS_ROOT:-artifacts/cross_modal_topology_localconf}"
LOCALCONF_SELECTION_ROOT="${LOCALCONF_SELECTION_ROOT:-artifacts/subset_selection_localconf}"
LOCALCONF_TRAIN_ROOT="${LOCALCONF_TRAIN_ROOT:-artifacts/subset_train_localconf}"
LOCALCONF_REPORT_ROOT="${LOCALCONF_REPORT_ROOT:-artifacts/reports}"

LOCALCONF_TAU_L="${LOCALCONF_TAU_L:-0.25}"
LOCALCONF_KAPPA_MIN="${LOCALCONF_KAPPA_MIN:-0.05}"
LOCALCONF_EPS="${LOCALCONF_EPS:-1e-8}"

TIMESTAMP="$(date '+%Y%m%d_%H%M%S')"
RUN_LOG_DIR="${EXPERIMENT_LOG_ROOT}/direction1_localconf_${DATASET}_${TIMESTAMP}"
mkdir -p "${RUN_LOG_DIR}" "${LOCALCONF_REPORT_ROOT}"

run_cross_stage() {
  local variant="$1"
  local fusion_mode="$2"
  local cross_root="${LOCALCONF_CROSS_ROOT}/${variant}"
...
run_selection_and_train() {
  ...
  local selection_root="${LOCALCONF_SELECTION_ROOT}/${variant}"
  local train_root="${LOCALCONF_TRAIN_ROOT}/${variant}"
...
stage_log "Direction1-localconf pipeline start: dataset=${DATASET} budgets=${BUDGETS_STR} seed=${SEED}"

run_cross_stage "dir1_localconf_only" "intersection"
