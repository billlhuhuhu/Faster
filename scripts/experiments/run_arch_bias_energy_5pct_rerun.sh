...
run_ours_selection() {
  ...
  measure_command "ours_${RATIO_TAG}_seed${seed}_selection" "${measurement_path}" "${log_path}" "${PROJECT_ROOT}" \
    env \
      ...
      WAVELET_MAIN_LATEST_PROXY_BATCH_SIZE="${ARCH5_PROXY_BATCH_SIZE:-${WAVELET_MAIN_LATEST_PROXY_BATCH_SIZE:-4096}}" \
      WAVELET_MAIN_LATEST_PROXY_TARGET_BATCH_SIZE="${ARCH5_PROXY_TARGET_BATCH_SIZE:-${WAVELET_MAIN_LATEST_PROXY_TARGET_BATCH_SIZE:-4096}}" \
      WAVELET_MAIN_LATEST_LSRC_BATCH_SIZE="${ARCH5_LSRC_BATCH_SIZE:-${WAVELET_MAIN_LATEST_LSRC_BATCH_SIZE:-4096}}" \
