...
def load_ablation_table(data_path: Path, budget: float) -> pd.DataFrame:
    ...
    method_col = normalized.get("method", raw.columns[0])
    budget_col = (
        normalized.get("threshold")
        or normalized.get("budget")
        or normalized.get("ratio")
        or raw.columns[1]
    )
...
def draw_left_panel(ax: plt.Axes, table: pd.DataFrame) -> None:
    ...
    ax.axvline(full_mr, color=BLUE, linestyle="--", linewidth=1.4, zorder=1)
    ax.text(
        full_mr,
        -0.64,
    ...
    for y, method, mr in zip(y_positions, LEFT_ORDER, mr_values):
        ...
        marker_size = 70 if is_full else 66

        if not is_full:
            ax.hlines(y, mr, full_mr, color=color, linestyle=(0, (3, 2)), linewidth=1.25, zorder=2)
            ax.text(mr - 0.22, y, f"{mr:.2f}", ha="right", va="center", color=color, fontsize=14)
            ax.text(full_mr + 0.10, y, f"{mr - full_mr:.2f}", ha="left", va="center", color=color, fontsize=14)
...
def draw_heatmap(ax: plt.Axes, cax: plt.Axes, table: pd.DataFrame, fig: plt.Figure) -> None:
    ...
    ax.set_xticklabels([label for label, _ in METRIC_COLUMNS], fontsize=12)
    ax.xaxis.tick_top()
...
def create_ablation_chart(data_path: Path, output_path: Path, budget: float = 0.03) -> None:
    ...
    fig = plt.figure(figsize=(15.6, 6.8), facecolor="white")
    ...
    fig.suptitle(f"Flickr, budget = {budget:.2f}", y=0.955, fontsize=19)
