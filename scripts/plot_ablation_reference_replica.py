...
def draw_left_panel(ax: plt.Axes, table: pd.DataFrame) -> None:
    ...
    ax.set_xlabel("mR")
    ax.set_title("(a) Module Contribution Analysis", fontweight="bold", pad=28)
...
def draw_heatmap(ax: plt.Axes, table: pd.DataFrame, fig: plt.Figure) -> None:
    ...
    ax.set_title("(b) Metric-Level Retrieval Summary", fontweight="bold", pad=48)
...
def create_ablation_chart(data_path: Path, output_path: Path, budget: float = 0.03) -> None:
    ...
    fig.suptitle(f"Flickr, budget = {budget:.2f}", y=0.94, fontsize=19)
