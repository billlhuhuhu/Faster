...
def load_feature_cache(feature_dir):
    ...
    img_features = torch.load(feature_dir / "img_features.pt", map_location="cpu")
    txt_features = torch.load(feature_dir / "txt_features.pt", map_location="cpu")
...
def load_unified_graph(cross_modal_dir):
...
def compute_proxy_sample_costs(proxy_points, candidate_points, topology_points, graph_scores, degree_weight, topology_weight):
    ...
    total_cost = geometry_cost + float(topology_weight) * topology_cost - float(degree_weight) * graph_scores.astype(np.float32)
...
def build_single_row_cost(proxy_point, candidate_indices, representation, topology_targets, graph_scores, degree_weight, topology_weight):
...
def run_proxy_matching(
    ...
    selected_indices, debug = resolve_duplicate_assignments(
        proxy_points,
        candidate_indices,
...
def save_selection_outputs(
    ...
    if matching_bundle is not None:
        ...
        torch.save(
            {
                "candidate_indices": torch.tensor(matching_bundle["candidate_indices"], dtype=torch.long),
                "total_cost": torch.tensor(matching_bundle["cost_bundle"]["total_cost"], dtype=torch.float32),
                "geometry_cost": torch.tensor(matching_bundle["cost_bundle"]["geometry_cost"], dtype=torch.float32),
                "topology_cost": torch.tensor(matching_bundle["cost_bundle"]["topology_cost"], dtype=torch.float32),
            },
            matching_cost_path,
        )
...
def run_proxy_optimized_selection(args, representation, unified_graph):
    ...
    topology_targets = build_topology_targets(
        unified_graph,
...
def run_subset_selection(args):
    ...
    unified_graph, cross_modal_summary = load_unified_graph(cross_modal_dir)
