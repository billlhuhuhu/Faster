...
echo "[baseline-eval-only] config=${BASELINE_CONFIG}"
echo "[baseline-eval-only] methods=${BASELINE_METHODS}"
echo "[baseline-eval-only] budgets=${BASELINE_BUDGETS}"
echo "[baseline-eval-only] seeds=${BASELINE_SEEDS}"
echo "[baseline-eval-only] output_root=${BASELINE_ROOT}"
