...
Definition:
- GraNd score = per-sample gradient norm, usually measured early in training.

Multimodal pair-level adaptation:
- Compute gradient-norm proxy on image/text branches in a surrogate pair-training loop.
- Aggregate to pair-level score.

reproduction_status: faithful_but_practical (surrogate gradient norm)
"""
...
def select_subset(
    ...
    hist_img = outputs["history"]["grand_img"]
    hist_txt = outputs["history"]["grand_txt"]
    stacked_img = np.stack(hist_img, axis=0)
    stacked_txt = np.stack(hist_txt, axis=0)
    window = int(cfg.get("grand_epoch_window", min(3, stacked_img.shape[0], stacked_txt.shape[0])))
    ...
    return make_result(
        ...
        config=cfg,
        notes="Early-epoch gradient-norm proxy using branch-specific surrogate dynamics.",
    )
