...
from src.vl_distill_utils import shuffle_files, nearest_neighbor, load_or_process_file
from src.vl_distill_utils import get_random_images_texts
...
## zx
def mix_embedding_up(x, y, alpha=0.2):
    if alpha > 0:
    ...
    batch_size = x.size(0)
    index = torch.randperm(batch_size)  # Random shuffle indices
...
## zx
def mix_embedding_up_second(x, y, index, lam):
...
def main(args):
    ...
    if args.disabled_wandb:
        ...
        wandb.init(project='RepBlend-distillation', config=args, name=args.name+"_"+make_timestamp())
    
    args.dsa_param = ParamDiffAug()
    zca_trans = args.zca_trans if args.zca else None
    args.zca_trans = zca_trans
    args.distributed = False #torch.cuda.device_count() > 1
    ...
    image_syn, text_syn = get_random_images_texts(args.num_queries, train_dataset, args)
    ...
    img_expert_files = sorted(list(glob.glob(os.path.join(expert_dir, "img_replay_buffer_*.pt"))))
    txt_expert_files = sorted(list(glob.glob(os.path.join(expert_dir, "txt_replay_buffer_*.pt"))))
    ...
    for it in tqdm(range(start_it, args.Iteration + 1)):
        ...
        if it in eval_it_pool and (save_this_it or it % 1000 == 0):
            with torch.no_grad():
                save_dir = os.path.join(".", "logged_files", args.dataset, wandb.run.name.replace(':', '-'))
        ...
        ## zx
        img_student_encoder = student_net.image_encoder.to('cuda')
        ...
        index_list = [] # zx the list of index for mixup
        lam_list = []
        ...
        for _ in range(args.syn_steps): 
            ...
            x = img_student_encoder(x)

            ## zx mixup x_embedding y_embedding
            x, this_y, lam, index_formixup = mix_embedding_up(x, this_y, args.alpha_2)

            x = img_student_net(x.float(), flat_param=img_forward_params)
            ...
            index_list.append(index_formixup)
            lam_list.append(lam)
        ...
        for i in range(args.syn_steps):
            x = img_student_encoder(x_list[i])
            index_this = index_list[i]
            lam_this = lam_list[i]

            # zx
            x, this_y = mix_embedding_up_second(x, y_list[i], index_this, lam_this)
            
            x = img_student_net(x.float(), flat_param=img_student_params[i])
            x = x / x.norm(dim=1, keepdim=True)
            
            ## zx
            this_y = txt_student_net(this_y, flat_param=txt_student_params[i])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--eval_it', type=int, default=150, help='how often to evaluate')
    ...
    parser.add_argument('--lr_lr', type=float, default=1e-2, help='learning rate for updating... learning rate')
    ...
    parser.add_argument('--loss_type', type=str, default='WBCE', help='loss type')
    ...
    parser.add_argument('--force_save', type=bool, default=True, help='this will save images for 50ipc')
    ...
    parser.add_argument('--image_root', type=str, default='/home/qiqitao/Flickr30k/', help='location of image root')
    ...
    parser.add_argument('--alpha', type=float, default=3, help='alpha in LoRA')
    parser.add_argument('--alpha_2', type=float, default=1, help='mixup para')

    parser.add_argument('--lr_sim', type=float, default=10.0, help='learning rate for updating similarity mat learning rate')
