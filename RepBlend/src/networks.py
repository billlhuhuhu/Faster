...
class CLIPModel_full(nn.Module):
    def __init__(
        ...
        if args.only_has_image_projection:
            self.image_projection = ProjectionHead(embedding_dim=self.image_embedding, projection_dim=self.image_embedding).to('cuda') # zx
    ...
    def calculate_loss(self, image_features, text_features, epoch, similarity=None):

        use_image_project = True if self.args.only_has_image_projection else False
    ...
    def forward(self, image, caption, epoch, similarity=None):
        ...
        if self.distill:
            ...
            text_features = self.text_encoder(caption).to('cuda') 
        # text_features = caption if self.distill else self.text_encoder(caption) 

        loss, acc = self.calculate_loss(image_features, text_features, epoch, similarity)
