...
def main(args):
    ...
    if args.disabled_wandb == True:
        ...
        wandb.init(project='RepBlend-Buffer', config=args, 
    ...
    for it in range(0, args.num_experts):
        ...
        img_teacher_net = teacher_net.image_projection.to(args.device)
        ...
        for e in trange(args.train_epochs):
            ...
            if (e+1) % args.eval_freq == 0:
                ...
                score_val_i2t, score_val_t2i = epoch_test(testloader, teacher_net, args.device, args.only_has_image_projection, bert_test_embed) # zx
            ...
            # zx
            # if e+1 == args.train_epochs:
            #     torch.save(img_teacher_net.state_dict(), os.path.join(save_dir, "img_projection_pretrained_{}.pth".format(it)))
            #     print("Saving {}".format(os.path.join(save_dir, "img_projection_pretrained_{}.pth".format(it))))
            #     torch.save(txt_teacher_net.state_dict(), os.path.join(save_dir, "txt_projection_pretrained_{}.pth".format(it)))
            #     print("Saving {}".format(os.path.join(save_dir, "txt_projection_pretrained_{}.pth".format(it))))
...
def make_buffer_parser():
    ...
    parser.add_argument('--data_path', type=str, default='./data/Flickr30k/', help='dataset path') # seems useless
    ...
    parser.add_argument('--text_encoder', type=str, default='bert', choices=['bert', 'clip', 'distilbert','gpt1', 'bge'], help='text encoder')
